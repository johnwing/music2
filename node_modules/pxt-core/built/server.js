...
function expandHtml(html, params) {
    ...
    var server = http.createServer(function (req, res) {
        ...
        for (var _i = 0, dd_1 = dd; _i < dd_1.length; _i++) {
            ...
            if (nodeutil.fileExistsSync(filename)) {
                if (/\.html$/.test(filename)) {
                    var html = expandHtml(fs.readFileSync(filename, "utf8"), htmlParams);
                    sendHtml(html);
                }
                else {
                    sendFile(filename);
        ...
        if (webFile) {
            if (/\.html$/.test(webFile)) {
                var html = expandHtml(fs.readFileSync(webFile, "utf8"), htmlParams);
                sendHtml(html);
            }
            else {
                sendFile(webFile);
            ...
            if (m)
                pathname = m[2];
            var lang_1 = (opts["translate"] && ts.pxtc.Util.TRANSLATION_LOCALE)
                || opts["lang"]
                || opts["forcelang"];
            readMdAsync(pathname, lang_1)
                .then(function (md) {
                var mdopts = {
