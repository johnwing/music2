    ...
    export class DebuggerService extends jacdac.JDService
    {
        ...
        handlePacket(pkt : jacdac.JDPacket) : number
        {
            ...
            if (remote)
            {
                ...
                view = debugView(serviceClass);
            }
            else if (pkt.device_address == 0)
            {
                serviceString = DebuggerClassEnums[pkt.service_number];
                view = debugView(jacdac.JDServiceClass.CONTROL);
            }

            let packetEntry ={pkt: pkt, view: view, timestamp: Date.now(), crcStatus: crcString, serviceClass: serviceString}

            // add packets to the end, we repaint starting with latest first.
            this.packets.push(packetEntry);
